...
import {
  Download, Upload, FileJson, Loader2, FolderTree, FileArchive,
...
export function WorkspaceIODialog({
  ...
  // Sync the tab + selection whenever the dialog (re)opens with new opts.
  useEffect(() => {
    if (!open) return;
    setTab(initialTab);
    if (preselectPageId) {
      setSelected(new Set([preselectPageId]));
    }
  }, [open, initialTab, preselectPageId]);
  ...
  /** Indented page list (row pages hidden). */
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as WorkspaceIOTab)} className="w-full">
          ...
          {/* ─── Import ZIP ──────────────────────────────────── */}
          <TabsContent value="import-zip" className="space-y-4 mt-4">
            {!zipSummary && (
              <>
                <div
                  onDrop={dropZip}
                  ...
                  className={cn(
                    "rounded-lg border-2 border-dashed p-10 text-center cursor-pointer transition",
                    dragOverZip ? "border-brand bg-brand/10" : "border-border bg-card hover:bg-accent",
                    zipBusy && "pointer-events-none opacity-60",
                  )}
                >
                  <input
                    ref={zipInputRef}
                  ...
                  <p className="mt-2 text-sm font-medium">
                    {zipBusy ? "Uploading…" : dragOverZip ? "Drop to upload" : zipFile ? `Picked: ${zipFile.name}` : "Drag a .zip file here, or click to choose"}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                ...
                <div className="flex justify-end">
                  <Button onClick={runZipImport} disabled={!zipFile || zipBusy}>
                    {zipBusy ? <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" /> : <Upload className="h-3.5 w-3.5 mr-1" />}
                    {zipBusy ? "Importing…" : "Import ZIP"}
                  </Button>
                </div>
              </>
            )}
            ...
            {zipSummary && (
              <div className="space-y-2">
                ...
                <div className="flex justify-end pt-1">
                  <Button onClick={() => { resetZip(); handleClose(false); }}>Done</Button>
                </div>
              </div>
            )}
          </TabsContent>
