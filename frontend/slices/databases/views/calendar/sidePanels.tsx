      ...
      {(["month", "week"] as const).map((m) => (
        <Button
          ...
          className={cn(
            ...
            mode === m ? "bg-brand text-white font-medium hover:bg-brand hover:text-white" : "text-muted-foreground",
          )}
      ...
      <div className="divide-y divide-border max-h-40 overflow-y-auto">
        {rows.map((r) => (
          <div key={r.id} className="flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-accent/40 group">
            <Button variant="ghost" onClick={() => onOpenRow(r.id)} className="flex-1 h-auto px-0 py-0 text-left truncate text-xs font-normal hover:bg-transparent justify-start">
            ...
            <Button
              ...
              className="h-auto w-auto p-0 opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive hover:bg-transparent [&_svg]:size-3.5"
              aria-label="Delete"
            >
