  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" className="h-auto gap-1 rounded-md bg-card px-2 py-1 text-xs font-normal [&_svg]:size-3">
    ...
    <div className="rounded-lg border border-border bg-card divide-y divide-border max-h-48 overflow-y-auto">
      {pins.map((p) => (
        <div key={p.row.id} className="flex items-center gap-2 px-3 py-1.5 hover:bg-accent/50 text-xs group">
          <Button variant="ghost" onClick={() => onOpenRow(p.row.id)} className="h-auto min-w-0 flex-1 justify-start gap-2 p-0 text-left text-xs font-normal hover:bg-transparent">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="h-auto rounded p-0.5 text-muted-foreground opacity-0 group-hover:opacity-100 [&_svg]:size-3" aria-label="Row actions">
