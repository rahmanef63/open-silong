    ...
    <div className="inline-flex rounded-md border border-border bg-card p-0.5 text-xs w-full">
      {options.map(o => (
        <Button
          ...
          className={cn(
            "flex-1 h-auto rounded px-2 py-1 text-xs font-normal",
            value === o.value ? "bg-brand text-white font-medium hover:bg-brand hover:text-white" : "text-muted-foreground"
          )}
        >{o.label}</Button>
      ))}
    </div>
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" className="flex w-full h-auto items-center justify-between bg-card px-2 py-1 text-xs font-normal [&_svg]:size-3">
